namespace SqlProvider.Providers
        ...
        member this.GenerateQueryText(sqlQuery,baseAlias,baseTable,projectionColumns,isDeleteScript, _) =
                    ...
                    | (cond::conds) ->
                        let build op preds (rest:Condition list option) =
                            ...
                            preds |> List.iteri( fun i (alias,col,operator,data) ->
                                    ...
                                    ~~(sprintf "%s%s" prefix <|
                                        match operator with
                                        | SqlProvider.IsNull -> sprintf "%s IS NULL" column
                                        | SqlProvider.NotNull -> sprintf "%s IS NOT NULL" column
                                        | SqlProvider.In ->
                                            let text = String.Join(",",paras |> Array.map (fun p -> p.ParameterName))
                                            Array.iter parameters.Add paras
                                            sprintf "%s IN (%s)" column text
                                        | SqlProvider.NestedIn ->
                                        ...
                                        | SqlProvider.NestedNotIn ->
                                            let innersql, innerpars = data.Value |> box :?> string * IDbDataParameter[]
                                            Array.iter parameters.Add innerpars
                                            sprintf "%s NOT IN (%s)" column innersql
                                        | SqlProvider.NestedExists ->
                                            let innersql, innerpars = data.Value |> box :?> string * IDbDataParameter[]
                                            Array.iter parameters.Add innerpars
                                            sprintf "EXISTS (%s)" innersql
                                        | SqlProvider.NestedNotExists ->
                                            let innersql, innerpars = data.Value |> box :?> string * IDbDataParameter[]
                                            Array.iter parameters.Add innerpars
            ...
            let columns =
                let extracolumns =
                    ...
                    | [] -> SqlProvider.Common.Utilities.parseAggregates fieldNotation Oracle.fieldNotationAlias sqlQuery.AggregateOp
                    | g  -> 
                        ...
                        let res2 = SqlProvider.Common.Utilities.parseAggregates fieldNotation Oracle.fieldNotationAlias aggs |> List.toSeq
